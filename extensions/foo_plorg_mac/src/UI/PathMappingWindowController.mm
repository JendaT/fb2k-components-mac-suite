...
- (void)startScanning {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [self.fpliteFiles removeAllObjects];
        for (NSString *file in contents) {
            if ([file hasSuffix:@".fplite"]) {
                [self.fpliteFiles addObject:[self.playlistsDir stringByAppendingPathComponent:file]];
            }
        }

        self.totalCount = self.fpliteFiles.count;

        dispatch_async(dispatch_get_main_queue(), ^{
            self.progressBar.maxValue = self.totalCount;
            self.statusLabel.stringValue = [NSString stringWithFormat:@"Found %ld playlist files", (long)self.totalCount];
        });

        // Scan each file for unique paths
        for (NSString *fplitePath in self.fpliteFiles) {
            if (self.shouldStop) break;

            [self scanFpliteFile:fplitePath];
            self.scannedCount++;

            dispatch_async(dispatch_get_main_queue(), ^{
                self.progressBar.doubleValue = self.scannedCount;
                self.statusLabel.stringValue = [NSString stringWithFormat:@"Scanned %ld / %ld files, found %ld drives",
                    (long)self.scannedCount, (long)self.totalCount, (long)self.discoveredDrives.count];
            });
        }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.stopButton.title = @"Rescan";

            if (self.shouldStop) {
                self.statusLabel.stringValue = [NSString stringWithFormat:@"Scan stopped. Scanned %ld / %ld files, found %ld drives",
                    (long)self.scannedCount, (long)self.totalCount, (long)self.discoveredDrives.count];
            } else {
                self.statusLabel.stringValue = [NSString stringWithFormat:@"Scan complete. Found %ld drives in %ld playlists",
                    (long)self.discoveredDrives.count, (long)self.totalCount];
            }
        });
    });
}
...
- (void)scanFpliteFile:(NSString *)path {
    ...
    for (NSString *line in lines) {
        ...
        if (driveKey && !self.discoveredDrives[driveKey]) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self addMappingRowForDrive:driveKey suggestion:suggestion];
            });
        }
    }
}
