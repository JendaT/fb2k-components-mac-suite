...
- (void)viewDidLoad {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [weakSelf.treeModel syncWithFoobarPlaylists];
    });
...
- (IBAction)createFolder:(id)sender {
    ...
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        [weakSelf startEditingNode:newFolder];
    });
}
...
- (IBAction)createPlaylist:(id)sender {
    ...
    @try {
        ...
        __weak typeof(self) weakSelf = self;
        dispatch_async(dispatch_get_main_queue(), ^{
            [weakSelf startEditingNode:newPlaylist];
        });
    } @catch (...) {
    ...
    [openPanel beginSheetModalForWindow:self.view.window completionHandler:^(NSModalResponse result) {
        ...
        @try {
            ...
            if (selectedNode && selectedNode.isFolder) {
                ...
                NSInteger index = parent ? [parent.children indexOfObject:selectedNode] : [strongSelf.treeModel.rootNodes indexOfObject:selectedNode];
                if (index != NSNotFound) {
                    for (NSInteger i = parsedNodes.count - 1; i >= 0; i--) {
                        ...
                        if (parent) {
                            ...
                            [strongSelf.treeModel insertRootNode:node atIndex:index + 1];
                        }
                        imported++;
                    }
                }
                ...
                for (TreeNode *node in parsedNodes) {
                    [strongSelf.treeModel addRootNode:node];
                    imported++;
                }
            }
...
- (IBAction)importFromStrawberry:(id)sender {
    ...
    // Show preview dialog
    self.strawberryPreviewController = [[StrawberryImportPreviewController alloc] init];
    self.strawberryPreviewController.delegate = self;
    self.strawberryPreviewController.targetFolder = targetFolder;

    __weak typeof(self) weakSelf = self;
    [self.view.window beginSheet:self.strawberryPreviewController.window completionHandler:^(NSModalResponse returnCode) {
        weakSelf.strawberryPreviewController = nil;
    }];

    // Load data after sheet is shown
    dispatch_async(dispatch_get_main_queue(), ^{
        [weakSelf.strawberryPreviewController loadFromStrawberryDatabase];
    });
}

#pragma mark - StrawberryImportPreviewDelegate
...
- (void)strawberryImportDidComplete:(NSArray<StrawberryPlaylistItem *> *)selectedPlaylists
    ...
    for (StrawberryPlaylistItem *item in selectedPlaylists) {
        ...
        if (item.uiPath.length > 0) {
            ...
            TreeNode *parentFolder = [self getOrCreateFolderPath:item.uiPath folders:folders baseFolder:targetFolder];
    ...
    FB2K_console_formatter() << "[Plorg] Imported " << imported << " playlists with " << tracksQueued << " tracks from Strawberry (loading async)";
}

- (void)strawberryImportDidCancel {
    FB2K_console_formatter() << "[Plorg] Strawberry import cancelled";
}
...
- (IBAction)importFromDeaDBeeF:(id)sender {
    ...
    for (NSString *dbplFile in dbplFiles) {
        ...
        if (newPlaylistIndex != pfc_infinite && paths.count > 0) {
            // Add tracks asynchronously using process_locations_async
            addTracksToPlaylistAsync(newPlaylistIndex, [playlistName UTF8String], paths);
    ...
    FB2K_console_formatter() << "[Plorg] Imported " << imported << " playlists with " << tracksImported << " tracks from DeaDBeeF (loading async)";
}
...
- (NSArray<NSString *> *)parseDeaDBeeF:(NSString *)dbplPath {
    ...
    if (bytes[0] != 'D' || bytes[1] != 'B' || bytes[2] != 'P' || bytes[3] != 'L') {
        FB2K_console_formatter() << "[Plorg] Invalid DBPL magic in file: " << [dbplPath UTF8String];
        return @[];
    }

    // Track count at bytes 6-9 (uint32_t little-endian)
    uint32_t trackCount = bytes[6] | (bytes[7] << 8) | (bytes[8] << 16) | (bytes[9] << 24);
    ...
    for (NSUInteger pos = 10; pos + 8 < length; pos++) {
        ...
        if (bytes[pos] == 0x04 && bytes[pos + 1] == 0x00 &&
            bytes[pos + 2] == ':' && bytes[pos + 3] == 'U' &&
            bytes[pos + 4] == 'R' && bytes[pos + 5] == 'I') {

            // Read path length (2 bytes little-endian after ":URI")
            NSUInteger lenPos = pos + 6;
            if (lenPos + 2 > length) break;

            uint16_t pathLen = bytes[lenPos] | (bytes[lenPos + 1] << 8);
            NSUInteger pathStart = lenPos + 2;

            if (pathLen > 0 && pathLen < 4096 && pathStart + pathLen <= length) {
                // Check if it starts with /
                if (bytes[pathStart] == '/') {
                    NSString *path = [[NSString alloc] initWithBytes:&bytes[pathStart]
                                                              length:pathLen
    ...
    FB2K_console_formatter() << "[Plorg] Parsed " << paths.count << " paths from DBPL (expected " << trackCount << ")";
    return paths;
}
...
- (IBAction)importFromVox:(id)sender {
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        if (newPlaylistIndex != pfc_infinite && trackPaths.count > 0) {
            // Add tracks asynchronously using process_locations_async
            addTracksToPlaylistAsync(newPlaylistIndex, [playlistName UTF8String], trackPaths);
    ...
    FB2K_console_formatter() << "[Plorg] Imported " << imported << " playlists with " << tracksImported << " tracks from Vox (loading async)";
}
...
- (IBAction)importFromOldPlorg:(id)sender {
    ...
    [openPanel beginSheetModalForWindow:self.view.window completionHandler:^(NSModalResponse result) {
        ...
        strongSelf.pathMappingController.delegate = strongSelf;
        [strongSelf.pathMappingController beginScanningWithPlaylistsDir:playlistsDir
...
- (IBAction)exportTree:(id)sender {
    ...
    [savePanel beginSheetModalForWindow:self.view.window completionHandler:^(NSModalResponse result) {
        ...
        @try {
            NSString *yaml = [strongSelf.treeModel toYaml];
            NSError *error = nil;
