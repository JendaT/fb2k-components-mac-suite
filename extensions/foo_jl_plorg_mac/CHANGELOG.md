...
- **Drop files from Finder onto a playlist** to append them (thanks @Scannou, #36). Dragging files or folders onto a playlist node imports them and adds them to the end of that playlist; folders in your tree still just spring open on hover. The import goes through foobar2000's own incoming-file handling, so folder recursion, file-type filters and sorting behave exactly like a normal drop onto a playlist. Previously the tree accepted the drag and expanded folders, but refused the drop.
