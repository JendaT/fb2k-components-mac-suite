...
- **"metadb is corrupted" on restart after the repair dialog** (field-reported): the migrator and the relauncher were separate processes coordinating through the *existence* of the staged SQL file in `/tmp` - the relauncher reopened foobar2000 as soon as that file vanished. But file existence is not a lock, and it vanished for three reasons other than "migration finished": the migrator's own abort path deleted it, a second migrator finishing first deleted it while the first was still writing, and the relauncher's 15-minute bound expired on large databases. In each case foobar2000 reopened while `sqlite3` still held metadb mid-write, which foobar2000 reported as a corrupted database (recovering afterwards). The migration and the relaunch are now one process with strict ordering - migrate, verify, vacuum, then reopen - so the app cannot start while the database is being written.
- **Only one metadb migrator per session** (same root defect, observed live 2026-08-05 with two migrators concurrently copying the same backup file): repair runs on startup and on every `/Volumes` event, and the orphan-cache heuristic re-detects the same dead UUIDs until the migration actually runs, so every pass spawned another migrator sharing one staged SQL file. Later passes now only refresh the staged plan, which the single waiting migrator reads at quit.
- **metadb no longer parks at ~2.7x the size it needs**: migration copies rows to the live UUID then deletes the originals, freeing pages *inside* the file that SQLite never returns to the filesystem - so the database kept its peak footprint permanently (observed: 2.35 GB back up to 6.94 GB across three remount cycles in six days, 62% of it dead space). The migrator now VACUUMs after a successful migration, while foobar2000 is exited and it holds the database exclusively; skipped with a logged note when disk space is short.
- **Migrations are verified, and the backup no longer doubles disk usage forever**: the migrator runs `PRAGMA quick_check` after migrating, restores the pre-migration backup if it fails, and otherwise removes that backup - previously it was rewritten on every migration and never deleted.
