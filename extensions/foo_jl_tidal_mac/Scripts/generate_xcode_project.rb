#!/usr/bin/env ruby
# Generate Xcode project for foo_jl_tidal_mac
...
PROJECT_NAME = "foo_jl_tidal"
BUNDLE_ID = "com.foobar2000.foo-jl-tidal"
SDK_PATH = Fb2kSdk.path

# Version from shared/version.h (single source of truth)
COMPONENT_VERSION = Fb2kVersions.get("tidal")
BUILD_NUMBER = Fb2kVersions.get_build_number("tidal")
...
api_files = Dir.glob("src/API/*.{cpp,h,mm}").map { |f| File.basename(f) }
...
# Root src files (fb2k_sdk.h, Prefix.pch)
root_src_files = Dir.glob("src/*.{h,pch}").map { |f| File.basename(f) }
...
[core_files, api_files, services_files, ui_files, integration_files, root_src_files].flatten.each do |file|
...
[['Core', core_files], ['API', api_files], ['Services', services_files], ['UI', ui_files], ['Integration', integration_files]].each do |group, files|
  files.each do |file|
...
# Add root src file references
root_src_files.each do |file|
  file_type = if file.end_with?('.h')
    'sourcecode.c.h'
  elsif file.end_with?('.pch')
    'sourcecode.c.h'  # PCH files are treated as headers
  else
    'text'
  end
  pbxproj_content += "\t\t#{file_ref_uuids[file]} /* #{file} */ = {isa = PBXFileReference; lastKnownFileType = #{file_type}; path = #{file}; sourceTree = \"<group>\"; };\n"
...
# Add root src files to src group
root_src_files.each do |file|
