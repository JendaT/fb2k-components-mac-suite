...
//  foo_jl_tidal_mac
//
//  Input decoder for tidal:// URLs
//

#pragma once

#include "../fb2k_sdk.h"
#include "../Core/TidalModels.h"
#include <atomic>

namespace tidal {

// Input decoder for Tidal streaming URLs
// Wraps underlying HTTP decoder after resolving stream URL
class TidalInputDecoder : public input_decoder_v2 {
public:
    TidalInputDecoder();
    ~TidalInputDecoder();
...
class TidalInfoReader : public input_info_reader {
public:
    TidalInfoReader();

    // Open and fetch metadata
    void open(const char* p_path, abort_callback& p_abort);
...
class TidalInputEntry : public input_entry_v2 {
public:
