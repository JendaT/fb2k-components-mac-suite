...
JL_COMPONENT_ABOUT(
    "Artist Biography",
    BIOGRAPHY_VERSION,
    "Artist biography viewer for foobar2000 macOS\n\n"
    "Features:\n"
    "- Display artist biography from Last.fm\n"
    "- Show artist images from multiple sources\n"
    "- Similar artists and tags display\n"
    "- Automatic caching for offline use\n"
    "- Layout parameters: mode"
);

VALIDATE_COMPONENT_FILENAME("foo_jl_biography.component");

// UI Element service registration (embeddable view)
namespace {
    // Unique GUID for this component
    static const GUID g_guid_biography = {
        0x7A3E5B2D, 0x4C6F, 0x8E1A,
        {0x9B, 0xDE, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD}
    };
    ...
    public:
        service_ptr instantiate(service_ptr arg) override {
            @autoreleasepool {
                // Unwrap layout parameters if provided
                ...
                BiographyController* controller = [[BiographyController alloc] initWithParameters:params];
        ...
        bool match_name(const char* name) override {
            return strcmp(name, "Artist Biography") == 0 ||
        ...
        fb2k::stringRef get_name() override {
            return fb2k::makeString("Artist Biography");
        }

        GUID get_guid() override {
            return g_guid_biography;
        }
    };
...
// play_callback_static implementation for track changes
class biography_play_callback : public play_callback_static {
public:
    ...
    void on_playback_new_track(metadb_handle_ptr p_track) override {
        BiographyCallbackManager::instance().onPlaybackNewTrack(p_track);
    }

    void on_playback_stop(play_control::t_stop_reason p_reason) override {
        BiographyCallbackManager::instance().onPlaybackStop(p_reason);
    }
...
FB2K_SERVICE_FACTORY(biography_play_callback);

// playlist_callback_static implementation for selection changes
class biography_playlist_callback : public playlist_callback_static {
public:
    unsigned get_flags() override {
    ...
    void on_items_selection_change(t_size p_playlist, const bit_array& p_affected, const bit_array& p_state) override {
        ...
        if (pm.is_valid() && pm->get_active_playlist() == p_playlist) {
            BiographyCallbackManager::instance().onSelectionChanged();
        }
    }
    ...
    void on_playlist_activate(t_size p_old, t_size p_new) override {
        // When switching playlists, update biography based on new playlist's selection
        BiographyCallbackManager::instance().onSelectionChanged();
    }
...
FB2K_SERVICE_FACTORY(biography_playlist_callback);

// Initialize component on startup
class biography_init : public initquit {
public:
    void on_init() override {
        console::info("[Biography] Artist Biography component initialized");
    }
...
FB2K_SERVICE_FACTORY(biography_init);
