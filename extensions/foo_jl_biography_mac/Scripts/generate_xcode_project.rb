#!/usr/bin/env ruby
# Generate Xcode project for foo_jl_biography_mac
...
PROJECT_NAME = "foo_jl_biography"
BUNDLE_ID = "com.foobar2000.foo-jl-biography"
SDK_PATH = Fb2kSdk.path

# Version from shared/version.h (single source of truth)
COMPONENT_VERSION = Fb2kVersions.get("biography")
BUILD_NUMBER = Fb2kVersions.get_build_number("biography")
...
[core_files, ui_files, api_files, integration_files].flatten.each do |file|
...
[['Core', core_files], ['UI', ui_files], ['API', api_files], ['Integration', integration_files]].each do |group, files|
  files.each do |file|
