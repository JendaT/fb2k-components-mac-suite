...
| Testability: extract JSON->model parsing from JLTidalAPI | Medium | 2026-07-07 | ~10 endpoint methods inline their own items-loop parsing (favorites item unwrap, folder filtering, exact-ISRC match in getTrackByISRC). Extract into pure parser functions + tests; API becomes thin transport |
| Testability: extract playlist-sync algorithms | Medium | 2026-07-07 | Naming round-trip, folder-hierarchy building, add/remove set-diff, ISRC-match planning in TidalPlaylistSync.mm. Feed fb2k data via callback struct (GroupBuilder pattern); move fb2k orchestration out of Core/ |
| Testability: browser view-model | Low | 2026-07-07 | Extract search/pagination/debounce/navigation state (6-way JLTidalBrowseMode machine) from 2236-line JLTidalBrowserController. Biggest win, biggest diff |
